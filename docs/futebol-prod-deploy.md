...
- **⚠️ A cascata de evidências não é a única cópia verbatim.** Desde as migrations 161 e 162, a **regra de visibilidade** — o `least` que calcula o fim útil de cada versão do snapshot, mais o cruzamento com a vitrine — existe em **quatro** lugares: `get_futebol_value_history`, `get_futebol_fixture_value`, `get_futebol_oportunidades_publicadas` e `scripts/futebol-roi.mjs`. As guardas são `src/utils/futebol-nascimento-visivel.test.ts` e `src/components/placar/placar-foto-de-nascimento.test.ts`. Mexer na regra é mexer nas quatro.
