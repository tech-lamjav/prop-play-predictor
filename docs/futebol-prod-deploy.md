...
- **23 tabelas nativas** (espelho **escalar** dos marts BQ `smartbetting-dados.futebol`): `dim_leagues, dim_teams, fact_fixtures, fact_fixture_stats, fact_fixture_events, fact_fixture_lineups, fact_fixture_lineups_players, fact_fixture_player_stats, fact_h2h, fact_injuries_snapshot, fact_standings_snapshot, fact_team_season_stats, fact_odds_snapshot, fact_predictions_api, int_futebol_odds_devig, int_futebol_premissas_1x2, int_futebol_premissas_ou, int_futebol_premissas_ah, int_futebol_premissas_btts, int_futebol_premissas_dc, fact_value_opportunities, fact_value_opportunities_hist, fact_insumos_medidos`. Colunas `ARRAY<STRING>` (`evidencias`/`avisos`) e `RECORD` (`coverage`) **não** são materializadas — o sync as pula e as RPCs **remontam** evidências/avisos a partir dos booleans das `int_futebol_premissas_*`. `fact_value_opportunities_hist` é um **dbt snapshot** (`dbt_valid_from/dbt_valid_to/dbt_scd_id`) — append-only, não é `drop+create` de dado (só de shape).
- **Índices** das RPCs (1 por tabela, +1 composto em odds/fixtures/standings).
- **Helper** `public._futebol_team_form(...)` (SECURITY DEFINER).
- **28 funções no total**: 26 RPCs `public.get_futebol_*`, mais o helper `_futebol_team_form` e o `futebol_dia_brt` (SECURITY DEFINER, `search_path=''`, lendo `futebol.*`) + grants `anon`/`authenticated`/`service_role`. **A seção 6 do `.sql` é o índice**: um grant por função, sempre.
  > ⚠️ Os "verificado byte-a-byte" de julho acima **não valem mais**. Entre 091 e 104 o arquivo divergiu do banco onze vezes, e oito funções inteiras (agenda, catálogo, premissas, números, histórico do jogo) só entraram em 18/08, achadas por code review.
  > A guarda contra isso agora é automática e roda na suíte: **`src/utils/shape-file-futebol.test.ts`** falha se uma migration criar função que não está no `.sql`, se faltar grant, ou se o arquivo ganhar BOM. A **seção 8 do `.sql`** tem as consultas manuais que complementam, conferindo corpo e assinatura contra o banco vivo.
...
- **Novos mercados:** quando subir um mercado novo no BQ → adicionar a tabela em `FUTEBOL_SYNC_TABLES_ORDERED` (`data-engineering/src/config.py`) + criar a tabela nativa no `.sql` + estender as **3** RPCs de valor (`get_futebol_value_board`, `get_futebol_fixture_value` e `get_futebol_value_history` — join por `market` + strings de evidência, e a cascata está copiada verbatim nas três) + chip no front. Mercados hoje: **1X2, Gols (O/U), Handicap, BTTS, Dupla chance**.
- **⚠️ A cascata de evidências não é a única cópia verbatim.** Desde as migrations 162 e 163, a **regra de visibilidade** — o `least` que calcula o fim útil de cada versão do snapshot, mais o cruzamento com a vitrine — vive em **três RPCs**: `get_futebol_value_history`, `get_futebol_fixture_value` e `get_futebol_oportunidades_publicadas`. Mais `scripts/futebol-roi.mjs`, que a ADR 0003 obriga a andar junto, e o arquivo de provisionamento, que repete cada uma. Mexer na regra é mexer em todas.
  - **Contar cópias não é contar RPCs**, e confundir os dois já deixou uma guarda frouxa. Cada migration que REDEFINE uma dessas funções carrega a regra de novo, então o número de ocorrências de texto nas migrations cresce sem que apareça RPC nova. As guardas são `src/utils/futebol-nascimento-visivel.test.ts` e `src/components/placar/placar-foto-de-nascimento.test.ts`; a segunda conta ocorrências contra redefinições, justamente para não prender num número fixo.
