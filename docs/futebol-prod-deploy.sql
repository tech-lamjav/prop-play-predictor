...
-- Janela de data das RPCs de histórico: a tabela não tinha índice em kickoff_utc
-- e o planner varria as 10,5k linhas. A seleção PIT de 30 dias caiu de 85 ms p/ 6,3 ms.
...
-- ── 5b. Histórico point-in-time do board (ADR 0009) ──────────────────────────
-- A aba Histórico de /futebol/oportunidades mostrava a nota RECALCULADA: lia
-- `get_futebol_value_board()`, que não filtra data e reconstrói tudo a cada run.
-- Ela contabilizava acerto/erro de linhas que só existiram DEPOIS do apito e,
-- para as outras, mostrava um score que ninguém viu. Aqui vem a oportunidade
-- COMO FOI PUBLICADA: a versão do snapshot `fact_value_opportunities_hist` que
-- estava viva no apito.
--
-- Decisões travadas:
--   • Grão = 1 linha por `opportunity_key`. `DISTINCT ON` blinda o grão mesmo se
--     o snapshot algum dia produzir janelas sobrepostas.
--   • PIT estrito, sem tolerância: dbt_valid_from <= kickoff < dbt_valid_to
--     (`dbt_valid_to` nulo = versão aberta). Chave sem versão viva no apito não
--     aparece — não cai para a versão mais próxima.
--   • Só jogo cujo apito JÁ SOOU (`kickoff < now()`): antes do apito não existe
--     "versão viva no apito" (a versão aberta satisfaz o predicado à toa), e uma
--     chave que já saiu do board voltaria à tela como oportunidade viva.
--   • Janela p_from/p_to em DIA DE BRASÍLIA, não UTC: 21:30 BRT é 00:30 UTC do
--     dia seguinte, horário de metade do calendário brasileiro. Os limites são
--     convertidos UMA vez (sargable), não linha a linha.
--   • O `RETURNS TABLE` espelha campo a campo o de `get_futebol_value_board`,
--     para o front reaproveitar `FutebolValueBoardRow` inteiro. Ficam de fora,
--     como no board, `janela_deteccao` e `premissas_sem_dado` — existem na
--     tabela, não no contrato. ⚠️ O board do DEV está derivado e declara
--     `premissas_sem_dado`; o espelho aqui é o de PRODUÇÃO.
--   • `get_futebol_value_board` NÃO é tocada (nem assinatura, nem corpo).
--
-- ⚠️ DÍVIDA CONHECIDA: o cascata de `evidencias` abaixo é a TERCEIRA cópia
-- verbatim (as outras duas em `get_futebol_fixture_value` e
-- `get_futebol_value_board`). Mercado novo agora mexe em 3 RPCs, não 2 (§6 do
-- .md). Extrair para um helper exige mexer no board, que a issue #257 proíbe —
-- fica para quem puder tocar nas três de uma vez.
--
