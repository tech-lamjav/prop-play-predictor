...
8. [Próximos Passos](#próximos-passos)
   - [O que é Opcional vs Obrigatório](#o-que-é-opcional-vs-obrigatório)
...
- **Vite** - Build tool e dev server (não Next.js)
...
- **BigQuery (Google Cloud)** - Data warehouse para análises NBA
- **OpenAI API** - Processamento de IA (Whisper, GPT-4)

**Ferramentas:**
- **Docker** - Containers para Supabase local
- **Supabase CLI** - Gerenciamento local do Supabase

**Nota Importante**: Este projeto usa **React + Vite**, não Next.js. Se você está acostumado com Next.js, algumas coisas serão diferentes (não há SSR, não há `pages/` directory do Next.js, etc.).

### Workflow de Desenvolvimento

**IMPORTANTE**: Para contribuir com o projeto:

1. **Clone o repositório principal** (não crie um fork separado para desenvolvimento)
   ```bash
   git clone <repository-url>
   cd prop-play-predictor
   ```

2. **Crie uma branch nova** para sua feature
   ```bash
...
**Não é necessário** configurar o Betinho (Edge Functions do Telegram/WhatsApp) para desenvolvimento, a menos que você esteja trabalhando especificamente nessa feature. A documentação do Betinho serve apenas como referência arquitetural.
...
**Obrigatório:**
- Nenhuma conta externa é necessária para desenvolvimento local básico

**Opcional (depende do que você está desenvolvendo):**
- Conta no **Google Cloud Platform** (apenas se estiver trabalhando na plataforma de análises NBA)
- Conta no **Supabase** (apenas para deploy/produção - não necessário localmente)
- **OpenAI API Key** (apenas se estiver testando o Betinho/Edge Functions)
...
1. **Clone o repositório** (não faça fork para desenvolvimento)
   ```bash
   git clone <repository-url>
   cd prop-play-predictor
   ```
...
3. **Configure o ambiente local** seguindo este guia
   - Você **NÃO precisa** configurar o Betinho (Edge Functions) a menos que esteja trabalhando nessa feature
   - Foque apenas no que precisa para sua tarefa

4. **Desenvolva e teste localmente**
   - Siga o checklist de validação na seção [Ambiente Isolado para Testes](#ambiente-isolado-para-testes)
...
### O que é Opcional vs Obrigatório

**Obrigatório para desenvolvimento:**
- ✅ Supabase local rodando (`supabase init` + `supabase start`)
- ✅ Frontend configurado (`.env` com `VITE_SUPABASE_URL` e `VITE_SUPABASE_ANON_KEY`)
- ✅ Dependências instaladas (`npm install`)

**Opcional (depende do que você está desenvolvendo):**
- ⚪ Betinho/Edge Functions (só se estiver trabalhando nessa feature)
- ⚪ BigQuery (só se estiver trabalhando na plataforma de análises)
- ⚪ OpenAI API Key (só se estiver testando processamento de mensagens)
...
- **Supabase Troubleshooting**: Ver `SUPABASE_TROUBLESHOOTING.md`

**Diferenças Vite vs Next.js:**
- Vite não tem SSR (Server-Side Rendering) por padrão
- Não há `pages/` directory do Next.js - usamos `src/pages/`
- Não há `getServerSideProps` - usamos React Query para data fetching
- Hot Module Replacement (HMR) é mais rápido no Vite
