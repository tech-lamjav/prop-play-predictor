...
interface ProfitByMarketChartProps {
  bets: Bet[];
  formatValue?: (value: number) => string;
  chartHeight?: number;
}

export const ProfitByMarketChart: React.FC<ProfitByMarketChartProps> = ({
  bets,
  ...
  const data = useMemo(() => {
    ...
    settled.forEach((bet) => {
      ...
      byMarket.set(market, (byMarket.get(market) ?? 0) + profit);
    });
  ...
  if (data.length === 0) {
    return (
      <div className="terminal-container p-4 mb-6">
        <h3 className="section-title mb-2">LUCRO POR MERCADO</h3>
  ...
  return (
    <div className="terminal-container p-4 mb-6">
      <h3 className="section-title mb-4">LUCRO POR MERCADO</h3>
