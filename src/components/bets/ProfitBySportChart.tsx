...
interface ProfitBySportChartProps {
  bets: Bet[];
  /** Optional: format currency for tooltip (e.g. R$ 1.234,56) */
  formatValue?: (value: number) => string;
  /** Optional: chart container height (responsive) */
  chartHeight?: number;
}

export const ProfitBySportChart: React.FC<ProfitBySportChartProps> = ({
  bets,
  ...
  const data = useMemo(() => {
    ...
    settled.forEach((bet) => {
      ...
      bySport.set(sport, (bySport.get(sport) ?? 0) + profit);
    });
  ...
  if (data.length === 0) {
    return (
      <div className="terminal-container p-4 mb-6">
        <h3 className="section-title mb-2">LUCRO POR ESPORTE</h3>
  ...
  return (
    <div className="terminal-container p-4 mb-6">
      <h3 className="section-title mb-4">LUCRO POR ESPORTE</h3>
