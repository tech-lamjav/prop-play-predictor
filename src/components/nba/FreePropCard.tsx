...
export function FreePropCard({ layout = 'vertical' }: FreePropCardProps) {
  ...
  if (isLoading) {
    return (
      <div className={isHorizontal ? 'rounded-lg bg-white border border-line p-6' : 'rounded-lg bg-white border border-line p-4'}>
        <h3 className="section-title mb-3">OPORTUNIDADE GRÁTIS DO DIA</h3>
        <div className="animate-pulse space-y-2">
          <div className="h-4 bg-canvas-2 rounded w-3/4"></div>
          <div className="h-4 bg-canvas-2 rounded w-1/2"></div>
        </div>
      </div>
    );
  }
  ...
  const renderOppCard = (opp: DailyOpportunity, compact: boolean) => {
    ...
    const statusColor = opp.trigger_status.toLowerCase().includes('out') ? 'bg-rose-100 text-rose-700 border-rose-200'
      : opp.trigger_status.toLowerCase().includes('doubtful') ? 'bg-orange-100 text-orange-700 border-orange-200'
      : 'bg-amber-100 text-amber-700 border-amber-200';
    const triggerTextColor = opp.trigger_status.toLowerCase().includes('out') ? 'text-rose-700'
    ...
    return (
      <div
        ...
        onClick={handleClick}
        className="bg-canvas-2 rounded border border-forest/30 hover:border-forest transition-colors cursor-pointer p-3"
      >
        {/* Row 1: Photo + Name + Score */}
        <div className="flex items-center gap-2.5 mb-2">
          <div className="w-8 h-8 rounded-full overflow-hidden bg-canvas-2 border border-line shrink-0 flex items-center justify-center">
            <img
          ...
          <div className="flex-1 min-w-0">
            <div className={`font-bold text-ink truncate ${compact ? 'text-xs' : 'text-sm'}`}>
          ...
          <div className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg border shrink-0 ${
            (opp.score ?? 0) >= 80 ? 'bg-emerald-100 text-forest border-forest/40' : (opp.score ?? 0) >= 70 ? 'bg-amber-100 text-amber-700 border-amber-200/40' : 'bg-orange-100 text-orange-700 border-orange-200'
          }`}>
        ...
        <div className="flex items-center gap-2 mb-2">
          <span className={`font-bold bg-forest/10 text-forest border border-forest/30 px-1.5 py-0.5 rounded ${compact ? 'text-[10px]' : 'text-xs'}`}>
            {statLabel}
          </span>
        ...
        <div className={`${compact ? 'text-[10px]' : 'text-xs'} text-ink`}>
          ...
          <span className="font-bold text-ink">{opp.avg_sem?.toFixed(1)}</span>
          <span className="ml-1 font-bold text-ink">(+{opp.gap_pct?.toFixed(1)}%)</span>
        </div>
        {opp.line_value && (
        ...
        <div className="mt-3 pt-0">
          <div className="flex items-center justify-center gap-1.5 w-full py-2 text-xs font-bold text-white bg-forest rounded hover:bg-forest-soft transition-all">
        ...
        {total > 1 && (
          <div className="flex items-center gap-1">
            <button onClick={(e) => { e.stopPropagation(); setActiveIndex((i) => (i - 1 + total) % total); }}
              className="w-5 h-5 flex items-center justify-center rounded hover:bg-canvas-2/50 text-ink/50 hover:text-ink transition-colors">
              <ChevronLeft className="w-3 h-3" />
            </button>
            <span className="text-[10px] text-ink/40 font-mono tabular-nums">{activeIndex + 1}/{total}</span>
